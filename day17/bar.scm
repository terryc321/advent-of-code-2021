...
#|
now want number of distinct vx , vy 
|#
(define (seek target)
  (match target
    ((tx1 tx2 ty1 ty2) 
     (let ((distinct '())
       ...
       (letrec ((foo (lambda (vx vx-max vy)
			 ...
			 (when val
			   (cond
			    ((member (list vx vy) distinct) #f)
			    (#t (set! distinct (cons (list vx vy) distinct))
				(set! n-distinct (+ 1 n-distinct))
				(format #t "distinct : ~a~%" n-distinct)
				;;(format #t "distinct : ~a~%" distinct)
				)))			    
			 (cond
			  ((> vx vx-max)
			   (foo 1 vx-max (+ vy 1)))
